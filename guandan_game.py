...
from utils import cardsToDict, findAllCombs, getWildCard, updateCardCombsAfterAction, generateNRandomCardLists, getChoiceUnderAction, getChoiceUnderThreeWithTwo,\
    checkCardCombTypeRank, getFlagsForActions, cardDictToModelList, indexOfCombBase, flattenFlags
...
class Cards(object):
    ...
    def init_cards(self, card1: List[str], card2 : List[str], level: int) -> None:
        ...
        self.last_move = CardComb.pass_cardcomb()
    ...
    def can_current_player_play_combbase(self, base : CombBase) -> bool:
        if base.is_pass():
            return not self.last_move.is_pass()
        if self.current_player == 1:
            for comb in self.comb1:
                if checkCardCombTypeRank(comb, base):
                    return True
        else:
            for comb in self.comb2:
                if checkCardCombTypeRank(comb, base):
                    return True
        return False
    ...
    def current_states(self, current_player : bool) -> Tuple[nplist, nplist]:
        ...
        if index == 1:
            ...
            flags = flattenFlags(getFlagsForActions(self.comb1, self.last_move, self.level, False))
        else:
            hand_cards = cardDictToModelList(self.cd2, self.level)
            flags = flattenFlags(getFlagsForActions(self.comb2, self.last_move, self.level, False))
...
class GDGame2P:
    ...
    def start_self_play(self, player : MCTSPlayer) -> Tuple[List[List[nplist]], List[nplist], List[int], int, int]:
        ...
        while not self.game_over():
            ...
            # Filter All choices
            if action_base.is_pass():
                self.cards.do_action(CardComb.pass_cardcomb())
            else:
                ...
                if len(choices) == 1:
                    self.cards.do_action(choices[0])
                else:
